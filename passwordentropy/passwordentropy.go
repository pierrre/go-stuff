...
//
// The entropy is estimated as L * log2(N): L is the password length (in runes) and N is the size of the character pool the password is assumed to be drawn from.
//
// The pool is the sum of the character classes present in the password.
// Digits (0-9) contribute 10.
// Letters contribute 26 per case (lower and/or upper), unless the only letters used are in the hex range (a-f, A-F), in which case they contribute 6 per case: this avoids overestimating the entropy of hex strings such as tokens from encoding/hex or crypto/rand.Text.
// Any other character contributes 1 per distinct rune, so a password made of a single repeated symbol has an entropy of 0.
//
